...
var AQE = (function ( $ ) {
  ...
  var usgswIconURL = '/assets/img/rubber-duck.png'
  var usgswIcon = L.icon({
      iconUrl: wasteIconURL,
  ...
  function onEachFeature(feature, layer) {
	...
	var html = "<div><h4>" + item.station_nm + "</h4><table class='table table-striped' data-aqs_id='"+item.aqs_id+"'>"
	...
	for( var i in params ) {
		if( 
			...
			cells.push( [ params[i], item[i] ] );
	}

	while( cells.length != 0 ) {
		var thisrow = cells.splice(0,1);
		html += "<tr>";
		for( var i in thisrow )
			html += "<td><b>" + thisrow[i][0] + "</b></td><td>" + thisrow[i][1] + "</td>"
		html += "</tr>";
	}

	html += "<table id='measurements'></table>";
	...
	layer.on('click', ( function( id ) {
		return function( e ) {
			$.getJSON("/usgsw/"+id+".json", function(ret){
				var data = ret[0].data;
				for( i in data ) {
					$("#measurements").append( "<tr><td>" + data[i]['param']['parm_nm'] + "</td><td>" + data[i]['result_va'] + "</td></tr>" );
				}
			});
		}
	} )( item.site_no ) );
  ...
  function filterFeatures(feature, layer) {
    ...
    if(item.type == "aqe"){
      ...
      if(filter_selections["usgsw_sites"] == "true"){ show = true}
      else { show = false }
    }
    return show
  }
  ...
  function update_filters(){
	...
	//usgsw_sites
	filter_selections['usgsw_sites'] = $('input.filter-usgsw_sites:checked').val()
    // aqs specific
  ...
  function update_map(key){
	...
	if( key == 'usgsw_sites' ) {
		layersData[key] = layersData[ key ].splice( 0, 27 );
	}
