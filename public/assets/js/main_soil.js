...
var AQE = (function($) {
  ...
  var landfillIconURL = defaultIconURL;
  ...
  function initialize() {
    ...
    $.each(dataset_keys, function(n, key) {
      if ($(".filter-" + key + ":checked").length > 0) {
        $.post("/ckan_proxy/" + key + ".geojson", function(data) {
          console.log('loop through datasets data:' + data);
          layersData[key] = data;
          update_map(key);
        });
      }
    ...
    $(".submit-map-filters").on('click', function(event) {
      console.log('.submit-map-filters');
      ...
      $.each(dataset_keys, function(n, key) {
        ...
        if ($(".filter-" + key + ":checked").length > 0 || filter_selections[key] === true) {
          console.log('.filter-[key]:checked or filter_sel[key] === true')
          if (layersData[key] === undefined) {
            ...
            $.post("/ckan_proxy/" + key + ".geojson", post_data, function(data) {
              ...
              update_map(key);
              stop_loading(key);
              console.log('POST to /ckan_proxy/');
            });
          } else {
            console.log('layersData[key] != undefined');
            start_loading(key);
            update_map(key);
            stop_loading(key);
          }
        } else {
          console.log('filter not checked or not true')
          update_map(key);
        }
      });
    });
  ...
  function onEachFeature(feature, layer) {
    ...
    if (item.type === "aqe") {
      ...
      html += "</div>";
      layer.bindPopup(html);
    } else if (item.type === "landfillsgeojson") {
      layer.setIcon(wasteIcon);
      var html = "<div><h4>" + item.name + "</h4>";
      if (item.Summary === "") {
        html += "No summary is available at this time";
      ...
      html += "</dv>";
      layer.bindPopup(html + "onEachFeature");
    } else {
      if (item.type) {
  ...
  function filterFeatures(feature, layer) {
    ...
    if (item.type === "aqe") {
      ...
      if (filter_selections["active-sites"] === "true" && item.status === "Active") {
        show = true;
      } else {
        show = false;
      }
    ...
    } else if (item.type === "landfills" || item.type === "landfillsgeojson") {
      if (filter_selections["landfills"] === "true" || filter_selections["landfillsgeojson"] === "true") {
        show = true;
      } else {
        show = false;
      }
  ...
  function update_filters() {
    ...
    // landfill
    filter_selections["landfills"] = $('input.filter-landfills:checked').val();
    filter_selections["landfillsgeojson"] = $('input.filter-landfillsgeojson:checked').val();
  }
  ...
  function update_map(key) {
    ...
    } else if (key === "landfillsgeojson") {
      console.log('update_map');

      for (var feature in layersData[key]['features']) {
        var feature = layersData[key]['features'][feature];
        var landfillFeature = {
          "type": feature["type"],
          "properties": {
            "NAME": feature["properties"]["SITE_NAME"],
            "FIPS": feature["properties"]["FIPS"],
            "SITE_STATUS": feature["properties"]["SITE_STATU"],
            "LANDFILL1_": feature["properties"]["LANDFILL1_"],
            "LANDFILL11": feature["properties"]["LANDFILL11"],
          },
          "geometry": {
        ...
        // console.log(landfillFeature);

        function createPopup (properties) {
          var html = "<div><h4>" + properties.NAME + "</h4>";
          html += "<table class='table table-striped'>";
          for (var property in properties) {
            html += "<tr><td>" + property + "</td><td>" + properties[property] + "</td></tr>";
          }
          html += "</table>";
          html += "</div>";
          return html;
        ...
        var landfillLayer = L.geoJson(landfillFeature, {
      ...
      function graphAQSHistoricalData() {
        ...
        $.getJSON(location.pathname + ".json?include_recent_history=1", function(data) {
          ...
          $('#dashboard-aqs-chart').highcharts({
            ...
            yAxis: [
            {
              title: {text: 'ppb (parts per billion)'},
              min: 0
            },
            {
              title: {text: ''},
      ...
      function authenticateAndLoadAsthmaHeatLayer(username, password) {
        if (username !== "" && password !== "" ) {
          $.post('/asthmaheat', {username: username, password: password}).done( function(data) {
            ...
            var propellerhealth_layer_bounds = [
              [37.8419378866983038, -86.0292621133016979],
              [38.5821425225734487, -85.1883896469475275]
            ];
            var propellerhealth_layer = L.layerGroup([
                L.imageOverlay(propellerhealth_layer_url,
                  propellerhealth_layer_bounds,
                  {
                    opacity: 0.8,
